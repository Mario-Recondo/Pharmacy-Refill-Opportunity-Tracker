...
| checked_out_at          | TIMESTAMP NULL                  | UTC ISO timestamp stamped when a row enters `Checked Out`; migration 009 backfills it from the latest checkout event or, when no event exists, due-date noon UTC |
...
Append-only workflow log (added by migration 004, 2026-07-15; ADR 0002): `id, refill_id, at, kind, old_value, new_value, profit`. Kinds: `refill_note`, `call_note`, `status` (technician edits) plus `followup_entered` / `followup_left` (span markers written by the Req Follow Up sweep — launch, any data change, day rollover; idempotent, never mutates refills). Values are **display names captured at event time**, so renaming a lookup never rewrites history. `profit` snapshots `new_profit` on `status → Checked Out` events as the historical Activity record of what was verified at that moment. The monthly total does not read this snapshot: it sums live `new_profit` for currently Checked Out rows, bucketed by the row's `checked_out_at` sold timestamp. Deliberately **no FK cascade**: events outlive row deletion as the permanent analytics record. Changes to the same field within a **2-minute settling window** collapse into one event (a set-then-revert inside the window nets to no event) — accidental clicks corrected on the spot don't pollute the record, but anything that survives 2 minutes is real history: "call note LVM+RSL, patient called back, P/U two minutes later" records both steps (user decision 2026-07-15, replacing an earlier same-calendar-day collapse that erased exactly that sequence). Shown read-only, with date and time, in the drawer's Activity section.

**Possible future extension — richer per-field history** (idea noted 2026-08-17). The drawer's field edits are moving from mutating the row object in place to *immutable row snapshots*: each successful edit produces a new `RefillRow` object that replaces the old one by `id` in the parent's list (done to satisfy the React Compiler lint rules in `eslint-plugin-react-hooks` v7). Today those snapshots are discarded once they update the grid, but *keeping* the before/after pair would be a natural foundation for a more detailed change history than `refill_events` captures now — e.g. per-field before→after diffs, or the "link sent twice" / per-attempt detail that §8 Q7 leaves open. This is groundwork only; nothing here builds that feature, and pursuing it would warrant its own ADR.
...
**Workflow event log.** Changes to refill note, call note and status append to `refill_events` (§4.5), with a 2-minute settling window so accidental set-then-fix edits leave at most one event (a full revert inside the window leaves none) while genuine same-day sequences all record. The Req Follow Up sweep reconciles `followup_entered`/`followup_left` span events against current membership. This log — not any status field — remains the historical source for Activity and event-based analytics such as time-in-follow-up and re-entry counts; the monthly profit total is the deliberate exception and reads live `new_profit` bucketed by `checked_out_at`.

**Monthly profit total.** A row entering `Checked Out` is stamped with `checked_out_at` by the status-update path; manually created Checked Out rows are stamped on insert. Repeated writes of the same status do not restamp it, and leaving Checked Out preserves the historical timestamp while removing the row from the live total. Migration 009 backfills the latest status-event timestamp where available and otherwise uses due-date noon UTC for older rows. The Month toolbar shows the sum of live `new_profit` for currently Checked Out rows whose `checked_out_at` falls in the displayed local calendar month (sold-month, not due-month), independent of filters. Rows with no New Profit are excluded and show a persistent circled `!` marker in the Status cell. A soft no-profit warning fires only for grid inline status edits, drawer edits, and drawer creates; Undo is deliberately exempt, with the marker as its backstop.
...
1. **Grid view (default screen).** Virtualized, editable data grid of refills for the selected month. Columns: Due date, Insurance (dropdown, colored), Drug, Rx # (click-to-copy), Refill Note (dropdown, colored), Call Note (dropdown, colored, gated), Old Copay, New Copay (tier-colored), Old Profit, New Profit (dynamic green), Status, Notes — plus an optional **Secondary** column (dropdown over `secondary_coverages`), **hidden by default** with a show/hide toggle, since the technician only sometimes wants to see secondary coverage (decided 2026-07-11). Month picker (with data-presence indicators, §3.1); quick filters for day, status, insurance, and "unresolved only." Sortable columns. Rx # (and optionally Drug) pinned so they stay visible and copyable during horizontal scroll (§5, "Floating Rx #") — the current sheet literally duplicates the RX column on the far right as a manual workaround, so this is a proven daily need. Within the month view, day boundaries get a visible separator between date groups (the sheet marks them with colored divider lines — a scanning habit worth preserving); separators show only while the grid is in due-date order and disappear under any other sort until date order returns (decided in UI sketching). Sortable columns include an always-visible **Reset sort** control restoring the default due-date order, so an accidental header click is cheap to undo.
   ...
   **Undo** (technician request, 2026-07-24). Ctrl+Z reverses field edits made in the grid or the drawer, up to 25 deep within the session. It applies immediately on the row the technician is on; an entry belonging to a different row warns and requires a second deliberate press to cross, and key auto-repeat is ignored — the technician's own concern was that spamming Ctrl+Z would silently revert rows they were not watching. Each undo scrolls to the row, flashes the changed cells, and names what it restored, since the affected row may be on another tab or month. A refill-note change that cleared a call note undoes as one unit. Undo replays through the same atomic write as the original edit, so the event log gains a compensating entry rather than losing history. Deletion remains permanent (see "Row deletion" below). ADR 0005 records why AG Grid's own `undoRedoCellEditing` is unusable here: its stack identifies rows by grid index, which this app's constant re-sorting turns into silent corruption of an unrelated refill.
   ...
   **Due-date lock** (technician feedback, 2026-07-11): a lock toggle next to the sort controls pins due date (ascending or descending, whichever is active when locked) as the fixed primary sort key. While locked, clicking any other column header sorts rows *within each day* — due date stays the outer order and the clicked column becomes the secondary key (with the usual asc/desc toggling) — so the day sections and their separator lines stay intact while the technician ranks rows inside each day (e.g., by Old Profit to work a day's most valuable refills first). Unlocking returns to normal single-column sorting; **Reset sort** also clears the lock. Refill Note cells showing `Nimble Link` additionally carry the aging counter described in §5.
   ...
   Grid spike learnings (2026-07-11), for implementation: (a) AG Grid computes row classes only on draw — call `redrawRows()` on sort change or the day-separator lines stick to stale rows; (b) drag-fill handle and multi-cell range copy/paste are Enterprise-only — bulk edits use checkbox selection + apply-to-selected instead, matching the import error-list pattern; (c) grid height ~75% of the window per technician feedback on the spike. The 2026-07-23 click model is coordinated by one app-level interaction state machine (`idle` / `selected` / `editing-inline` / `editing-overlay`) and one capture-phase outside-click listener, not independent per-cell listeners. (d) **`singleClickEdit` is unusable in this app and edits start through `api.startEditingCell` instead** (found 2026-07-24 by tracing live events): AG Grid's `shouldStartEditing` opens an editor only when `event.detail === 1`, and the browser keeps incrementing that click counter while consecutive clicks stay close in time and space. Under `singleClickEdit` every rapid re-click is therefore refused — including the second click the overlay outside-click rule *requires*, which locked the technician out until they paused or moved the mouse. `startEditingCell` never consults the counter. (e) **Dropdown popups set `cellEditorPopupPosition: "under"` and options refuse pointer selection until the pointer has moved onto the popup** (both added 2026-07-24). AG Grid's default `'over'` renders the popup on top of the cell that opened it — directly beneath the cursor — so the next click landed on an option and committed it: one rapid click sequence silently wrote four different insurances to one row before it was caught. Positioning the popup below the cell keeps it clear of the cursor, and the movement requirement covers the case where AG Grid flips it back over the cell near the viewport bottom. Keyboard selection is unaffected.
