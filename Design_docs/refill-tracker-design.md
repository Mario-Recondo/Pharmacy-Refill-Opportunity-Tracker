...
- **Shifted-Secondary quirk (verified against the real July 2026 full export, 2026-07-20).** When a row has no secondary coverage, PioneerRX emits the patient-paid amount **in the `Secondary` column** and leaves `Patient Paid Amount` blank (150/150 cross-checked rows matched the true copay; rows with a real secondary name are laid out normally). The parser therefore treats a numeric `Secondary` value as never being a plan name: it is rerouted to `old_copay` when the copay is blank, and the row's secondary is imported as none.
