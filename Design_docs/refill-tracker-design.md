...
- Pioneer exports arrive as `.xlsx` files with Excel date cells; CSV remains accepted. The fixed technician contract is `Days Supply Ends On`, `Rx Number`, `Dispensed Item Name`, `Patient Paid Amount`, `Net Profit`, `Refills Left`, `Primary`, and `Secondary`; `Dispensed Item NDC` is deferred. Import is date-driven and never silently imports rows: choose file → map columns → resolve names → preview → one atomic commit.
