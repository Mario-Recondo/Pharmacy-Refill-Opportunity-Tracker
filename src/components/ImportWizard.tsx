...
import {
  buildCommitPlan, computeDispositions, normalizeName, parseRows, proposeMapping, resolveNames, unresolvedAttentionCount, validateMapping, validatePlan,
...
export default function ImportWizard({ lookups, visibleMonth, onClose, onChanged }: {
  ...
  const setNameChoice = (name: NameResolution, value: string) => {
    setResolutions((all) => all.map((r) => r === name ? value === "create" ? { ...r, choice: "create", targetName: r.rawName } : value === "blank" ? { ...r, choice: "blank" } : { ...r, choice: "existing", targetId: Number(value) } : r));
  };
